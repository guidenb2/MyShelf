...
* [`src/myshelf_application/`](src/myshelf_application/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/myshelf_application/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `myshelf-application` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/myshelf_application/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns myshelf-application.example
  (:require [myshelf-application.config :as config])
